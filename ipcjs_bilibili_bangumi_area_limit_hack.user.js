...
/**
需安装Content Security Policy Override这类插件关闭b站搜索页面的CSP策略,防止浏览器将代理服务器地址发送给B站
安全起见,建议配置插件在特定网站上,填入*://*.bilibili.com/*
在插件配置页面粘贴以下内容即可
[
    ...
    ***************************************************************************** */
    ...
    function redirect_to_bangumi_or_insert_player() {
        ...
        function tryRedirectToBangumiOrInsertPlayer() {
            ...
            if(document.getElementsByClassName("big-btn go-home go-whatever")) document.getElementsByClassName("big-btn go-home go-whatever")[0].dispatchEvent(new MouseEvent("click"));//取消错误页面自动跳转首页
    ...
    function fixBangumiPlayPage() {
        util_init(() => __awaiter(this, void 0, void 0, function* () {
            ...
            if (util_page.new_bangumi()) {
               ...
               //     (_h = $danmukuBox.parentElement) === null || _h === void 0 ? void 0 : _h.replaceChild($template.content.firstElementChild, $danmukuBox.nextSibling);
               // }
            }
        }));
    }
            ...
            modifyGlobalValue('__NEXT_DATA__', {
                onWrite: (value) => {
                    ...
                    for (const query of queries) {
                        const data = query.state.data.seasonInfo;
                        data.rights.area_limit = false;
                        data.rights.allow_dm = 1;
                        switch (query.queryKey[0]) {
                            case 'pgc/view/web/season':
                                // 最重要的一项数据, 直接决定页面是否可播放
                                if (data.epMap) Object.keys(data.epMap).forEach(epId => removeEpAreaLimit(data.epMap[epId]));
    ...
    function updateLoginFlag(loadCallback) {
        Async.jsonp(balh_config.server_custom + '/login?act=expiretime')
            .then(() => loadCallback && loadCallback(true)).catch((e)=>{});
        ...
        switch (e.data.split(':')[0]) {
            case 'BiliPlus-Login-Success': {
                ...
                Promise.resolve('start')
                    .then(() => Async.jsonp(balh_config.server_custom + '/login?act=getlevel'))
...
var css$1 = "#balh-settings {\n  font-size: 12px;\n  color: #6d757a; }\n  #balh-settings h1 {\n    color: #161a1e; }\n  #balh-settings a {\n    color: #00a1d6; }\n  #balh-settings a:hover {\n    color: #f25d8e; }\n  #balh-settings input {\n    margin-left: 3px;\n    margin-right: 3px; }\n  #balh-settings label {\n    width: 100%;\n    display: inline-block;\n    cursor: pointer; }\n  #balh-settings label:after {\n    content: \"\";\n    width: 0;\n    height: 1px;\n    background: #4285f4;\n    transition: width .3s;\n    display: block; }\n  #balh-settings label:hover:after {\n    width: 100%; }\n  #balh-settings form {\n    margin: 0; }\n  #balh-settings input[type=\"radio\"] {\n    appearance: radio; }\n  #balh-settings input[type=\"checkbox\"] {\n    appearance: checkbox; }\n\n@keyframes balh-settings-bg {\n  from {\n    background: rgba(0, 0, 0, 0); }\n  to {\n    background: rgba(0, 0, 0, 0.7); } }\n";
            ...
            if (indexNav == null) {
                ...
                if (util_page.bangumi_md()) {
                    ...
                    if (navTools) {
                        ...
                        if (document.getElementById("ogv-weslie-media-info-follow")) {
                            let navTools =document.getElementById("ogv-weslie-media-info-follow").previousSibling;
                            const _indexNav = indexNav = document.body.appendChild(createElement('div', { style: { position: 'fixed', right: '6px', bottom: '350px', zIndex: '129', textAlign: 'center'} }));
                            indexNav.appendChild(createBtnStyle('40px'));
                        }
                    }
                }
                if (indexNav) {
        ...
        if (window === window.top) {
            window.addEventListener('message', (event) => {
                if (event.data === 'balh-show-setting') {
                    _showSettings();
                    document.getElementById('upos-server').value = balh_config.upos_server || '';
                }
            });
        }
    ...
    function injectFetch() {
        ...
        window.fetch = function (input, init) {
            if (!(balh_config.mode === r.const.mode.REDIRECT || (balh_config.mode === r.const.mode.DEFAULT))) return originFetch(input, init).then(r => {return r;});
            ...
            }else if (new URL("https:" + input).searchParams.get('mid') == "0" || new URL("https:" + input).searchParams.get('up_mid') == "0" || new URL("https:" + input).searchParams.get('vmid') == "0") {
                //出差空间,大量请求的mid相关参数为0,比如up_mid,vmid
                let mid = location.pathname.slice(1);
                input = input.replace("mid=0","mid="+mid);
            }
        ...
        window.fetch = Async.wrapper(window.fetch, resp => new Proxy(resp, {
            get: function (target, prop, receiver) {
                if (!(balh_config.mode === r.const.mode.REDIRECT || (balh_config.mode === r.const.mode.DEFAULT))) return target[prop];
                util_debug('injectFetch:', target.url);
                if (prop === 'json') {
                    return Async.wrapper(target.json.bind(target), oriResult => {
                        util_debug('injectFetch json:', target.url);
                        if (target.url.match(RegExps.urlPath('/x/space/wbi/acc/info')) && oriResult.code === -404 ) {
                            let mid = new URL(target.url).searchParams.get('mid');
                    ...
                    return Async.wrapper(target.text.bind(target), oriResult => {
                        util_debug('injectFetch text:', target.url);
                        return oriResult;
                    }, error => error);
                }/**else if (prop === 'data') {
                    //文档上没看到data,有时候为空,有时候是obj
                    if(!target[prop]) {return target.data};
                    if(typeof target[prop] === 'function') {
                        return Async.wrapper(target.data.bind(target), oriResult=>{
                        util_debug('injectFetch data:', target.url);
                        if (target.url.match(RegExps.urlPath('/x/space/wbi/acc/info')) && oriResult.code === -404) {
                            let mid = new URL(target.url).searchParams.get('mid');
                            if (space_account_info_map[mid]) {
                                return space_account_info_map[mid]
                        ...
                        if (target.url.match(RegExps.urlPath('/x/space/wbi/acc/info')) && target.data.code === -404 ) {
                            let mid = new URL(target.url).searchParams.get('mid');
                            if (space_account_info_map[mid]) {
                                return space_account_info_map[mid]
                            }
    ...
    function scriptContent() {
        ...
        ((function () {
            ...
            function injectXhr() {
                injectXhrImpl({
                    ...
                    transformRequest: ({ url, container }) => {
                        ...
                        }else if (new URL("https:" + url).searchParams.get('mid') == "0" || new URL("https:" + url).searchParams.get('up_mid') == "0" || new URL("https:" + url).searchParams.get('vmid') == "0" ) {
                            //请求中mid参数为0,转给fetch
                            return window.fetch("https://"+ url).then(r=>{return r.json()}).then(r=>{return r});
                        }
                        return null
                    }
                });
            ...
            function isBangumiPage() {
                return isBangumi(Func.safeGet('window.__INITIAL_STATE__ ? (window.__INITIAL_STATE__.mediaInfo.season_type || window.__INITIAL_STATE__.mediaInfo.ssType):null'))
            }
